...
class _CustomCalendarScrollViewState extends State<CustomCalendarScrollView> with TickerProviderStateMixin {
  ...
  DateTime? _updateSelectedDate(KeyEvent event, _CalendarViewState currentViewState, _CalendarView currentView,
  ...
  KeyEventResult _updatePageUpAndDown(KeyEvent event, _CalendarViewState currentViewState, bool isResourceEnabled) {
  ...
  KeyEventResult _updateAppointmentSelection(KeyEvent event, _CalendarViewState currentVisibleViewState,
    ...
    if (HardwareKeyboard.instance.isShiftPressed) {
      if (event.logicalKey == LogicalKeyboardKey.tab) {
        ...
        return _updateAppointmentSelectionOnView(selectedAppointment, currentVisibleViewState, isAllDay,
            isResourceEnabled, !HardwareKeyboard.instance.isShiftPressed);
      }
  ...
  KeyEventResult _onKeyDown(FocusNode node, KeyEvent event) {
    ...
    if (HardwareKeyboard.instance.isControlPressed && widget.view != CalendarView.schedule) {
...
class _CalendarViewState extends State<_CalendarView> with TickerProviderStateMixin {
  ...
  void _scrollToPosition() {
    SchedulerBinding.instance.addPostFrameCallback((_) {
      ...
      if (scrollPosition == -1 || (_scrollController != null && _scrollController!.position.pixels == scrollPosition)) {
        return;
      }
      if (_scrollController != null) {
  ...
  void _addMouseHoverForMonth(Canvas canvas, Size size, double xPosition, double yPosition, double width) {
    if (xPosition + (width / 2 - _dayTextPainter.width / 2) <= viewHeaderNotifier.value!.dx &&
      ...
      _drawTodayCircle(canvas, xPosition + (width / 2 - _dayTextPainter.width / 2), yPosition, _dayTextPainter,
          hoveringColor: (themeData.brightness == Brightness.dark ? Colors.white : Colors.black87).withOpacity(0.04));
    }
  }
...
class _TimeRulerView extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (isTimelineView) {
      ...
      canvas.drawLine(Offset(0, size.height), Offset(xPosition, size.height), _linePainter);
    } else {
  ...
  void _drawTimeLabels(Canvas canvas, Size size, DateTime date, double hour, double xPosition, double yPosition,
    ...
    final List<String> timeFormatStrings = CalendarViewHelper.getListFromString(timeSlotViewSettings.timeFormat);
    ...
    for (int i = isTimelineView ? 0 : 1; i <= (isTimelineView ? horizontalLinesCount - 1 : horizontalLinesCount); i++) {
      ...
      _textPainter.paint(
          canvas, Offset(startXPosition + ((cellWidth - _textPainter.width) / 2 - padding), startYPosition));
...
class _CurrentTimeIndicator extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final double currentTimePosition = CalendarViewHelper.getTimeToPosition(
        Duration(hours: getLocationDateTime.hour, minutes: getLocationDateTime.minute),
